...
/**
 * How long a concurrency gate waits for its participants before giving up.
 *
 * This is a failure ceiling, not a delay: the gate releases as soon as every party
 * arrives, so a larger budget never slows a passing test. One second was sized for a
 * local database; against a remote one each participant needs several round trips to
 * reach the gate, and the gate timed out on tests whose behaviour was correct.
 */
const GATE_TIMEOUT_MS = 30_000;

function createBoundedPause(timeoutMs = GATE_TIMEOUT_MS): BoundedPause {
  let entered = false;
  let settled = false;
...
describe.skipIf(!enabled)(
  'DrizzleCaseService (database integration)',
  ...
  () => {
    ...
    it('atomically persists a standard claim without plaintext sensitive data', async () => {
      ...
      expect(aggregate.consumers[0]?.emailLookupHash).toBe(
        await crypto.lookupHash(normalizedEmail),
      );
    ...
    it('replays the original response for the same key and canonical request', async () => {
      ...
      const [stored] = await handle!.db
        ...
        .where(
          eq(
            idempotencyRecords.caseId,
            (await loadAggregate(handle!, first.caseReference)).case.id,
          ),
        );
    ...
    it('atomically recycles an expired Idempotency-Key for a new Draft', async () => {
      ...
      try {
        ...
        expect(records[0]?.caseId).toBe(
          (await loadAggregate(handle!, second.caseReference)).case.id,
        );
    ...
    it('never marks a Document for deletion after Claim association wins the Draft lock', async () => {
      ...
      try {
        ...
        // Claimed here so that bailing out before the allSettled below cannot leave
        // this rejection unhandled — which is what it did when the gate timed out.
    ...
    it('allows only one winner when one key is used concurrently for different Drafts', async () => {
      ...
      try {
        ...
        expect(
          (await countCasesForEmail(firstEmail)) + (await countCasesForEmail(secondEmail)),
        ).toBe(1);
      } finally {
        await cleanupClaimFixture(handle!, secondFixture);
      }
    ...
    it('fails after three public Case Reference collisions and leaves the Draft active', async () => {
      ...
      const collisionCaseIds = await insertReferenceCollisions(
        fixture!.draftId,
        collisionReferences,
      );
