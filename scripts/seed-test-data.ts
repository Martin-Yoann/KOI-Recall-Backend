...
async function main() {
  ...
  await db
    ...
    .values({
      ...
      summary:
        'The front wheel assembly on BabyJoy X1 strollers manufactured between January and May 2026 may detach during use, posing a fall and injury hazard to children.',
      hazard:
        'Front wheel axle pin can shear under lateral stress, causing the wheel assembly to separate from the frame. At least 12 incidents reported, including 3 involving minor injuries to children.',
      immediateAction:
        'Stop using the stroller immediately. Inspect the front wheel assembly for any looseness or unusual movement. Do not attempt to repair the wheel yourself.',
      remedySummary:
        'Free repair kit with reinforced axle pin and installation instructions, or full refund with return of stroller.',
      ...
      faq: [
        {
          ...
          question: 'How do I know if my stroller is affected?',
          answer:
            'Check the model number on the frame label under the seat. Affected models: BJ-X1-2026-A through BJ-X1-2026-D with date codes 01/2026 through 05/2026.',
        },
        {
          topic: 'Repair',
          question: 'Can I install the repair kit myself?',
          answer:
            'Yes. The repair kit includes detailed illustrated instructions and a video guide. If you prefer, authorized service centers will install it free of charge.',
        },
      ],
    })
    .onConflictDoNothing();
  ...
  await db
    ...
    .values({
      ...
      attributes: { weight: '7.8kg', color: 'Graphite Gray', ageRange: '0-36 months' } as Record<
        string,
        unknown
      >,
    })
  ...
  await db
    ...
    .values([
      {
        variantId: strollerVariantId,
        identifierType: 'sku',
        rawValue: 'BJ-X1-GRY-2026',
        normalizedValue: 'bj-x1-gry-2026',
      },
      {
        variantId: strollerVariantId,
        identifierType: 'unit_upc',
        rawValue: '0123456789050',
        normalizedValue: '0123456789050',
      },
      {
        variantId: strollerVariantId,
        identifierType: 'model',
        rawValue: 'BJ-X1-2026-A',
        normalizedValue: 'bj-x1-2026-a',
      },
    ])
    .onConflictDoNothing();
    ...
    .values([
      {
        campaignVersionId: strollerVersionId,
        code: 'repair_kit',
        displayName: 'Free Repair Kit — Reinforced Axle',
      ...
      },
      {
        campaignVersionId: strollerVersionId,
        code: 'refund',
        displayName: 'Full Refund with Return',
    ...
    .values([
      {
        ...
        instructions: 'Photo of the stroller frame label showing model number and date code.',
      },
      {
        campaignVersionId: strollerVersionId,
        ...
        maximumFileSizeBytes: 10485760,
        instructions: 'Receipt, invoice, or order confirmation.',
      },
    ])
    .onConflictDoNothing();
  ...
  await db
    ...
    .values({
      ...
      htmlBody:
        '<p>We received your recall claim. Your reference is <strong>{{caseReference}}</strong>.</p>',
  ...
  campaigns.push({
    ...
    code: 'CPSC-26-189',
    status: 'active',
    versionId: strollerVersionId,
    productId: strollerProductId,
    variantId: strollerVariantId,
    locale: 'en-US',
  });
      ...
      approvals: [
        {
          role: 'business' as const,
          approvedBy: 'lin-wei',
          approvedAt: new Date('2026-08-01T00:00:00.000Z').toISOString(),
        },
        {
          role: 'legal_compliance' as const,
          approvedBy: 'chen-mei',
          approvedAt: new Date('2026-08-01T12:00:00.000Z').toISOString(),
        },
      ],
    })
    .onConflictDoNothing();
  ...
  await db
    ...
    .values({
      ...
      summary:
        'HeatPro 1.7L Electric Kettles (model HP-EK17) manufactured between March and July 2026 may overheat due to a defective thermostat, posing burn and fire hazards.',
      hazard:
        'Thermostat may fail to auto-shutoff at boiling point, causing the kettle to continuously heat until the thermal fuse blows. In rare cases, the plastic base may melt, posing a fire risk. Eight reports of base melting; no injuries reported.',
      immediateAction:
        'Unplug the kettle and do not use it. Do not attempt to bypass the auto-shutoff feature.',
      remedySummary:
        'Full refund with proof of purchase, or free replacement with updated HP-EK17-V2 model.',
  ...
  await db
    ...
    .values([
      {
        variantId: kettleVariantId,
        identifierType: 'sku',
        rawValue: 'HP-EK17-SS-2026',
        normalizedValue: 'hp-ek17-ss-2026',
      },
      {
        variantId: kettleVariantId,
        identifierType: 'unit_upc',
        rawValue: '0123456789067',
        normalizedValue: '0123456789067',
      },
    ])
    .onConflictDoNothing();
  ...
  await db
    ...
    .values([
      ...
      },
      {
        campaignVersionId: kettleVersionId,
        code: 'replacement',
        displayName: 'Free Replacement (HP-EK17-V2)',
        requiresMailingAddress: true,
        sortOrder: 2,
      },
    ])
    ...
    .values([
      {
        ...
        instructions: 'Photo of the kettle base showing model label and batch sticker.',
      },
      {
        campaignVersionId: kettleVersionId,
        category: 'proof_of_purchase',
        ...
        allowedMimeTypes: ['image/jpeg', 'image/png', 'application/pdf'],
        maximumFileSizeBytes: 5242880,
        instructions: 'Receipt or online order confirmation.',
      },
    ])
    .onConflictDoNothing();
  ...
  campaigns.push({
    ...
    code: 'CPSC-26-312',
    status: 'scheduled',
    versionId: kettleVersionId,
    productId: kettleProductId,
    variantId: kettleVariantId,
    locale: 'en-US',
  });
  ...
  const existingCampaigns = await db
    ...
    .where(sql`${recallCampaigns.isTestData} = true`);
  const existingProducts = await db
    .select({ id: campaignProducts.id, campaignVersionId: campaignProducts.campaignVersionId })
  ...
  console.log(
    `    ✓ ${allCampaigns.length} campaigns (${allCampaigns.map((c) => c.code).join(', ')})`,
  );
  ...
  const consumerData = [
    {
      ...
      addr: '245 East 63rd Street, Apt 4B, New York, NY 10065',
    },
    {
      first: 'James',
      last: 'Wilson',
      ...
      addr: '1420 North Lake Shore Drive, Chicago, IL 60610',
    },
    {
      first: 'Emily',
      last: 'Davis',
      ...
      addr: '888 Howard Street, San Francisco, CA 94103',
    },
    {
      first: 'Amanda',
      last: 'Torres',
      email: 'atorres@icloud.com',
      phone: '+1 (305) 555-0176',
      addr: '1100 Biscayne Boulevard, Miami, FL 33132',
    },
    {
      first: 'Michael',
      last: 'Kim',
      email: 'michael.kim@proton.me',
      phone: '+1 (206) 555-0192',
      addr: '1918 8th Avenue, Seattle, WA 98101',
    },
    {
      first: 'Priya',
      last: 'Patel',
      ...
      addr: '1 Beacon Street, Boston, MA 02108',
    },
    {
      first: 'David',
      last: 'Thompson',
      ...
      addr: '1600 Broadway, Denver, CO 80202',
    },
    {
      first: 'Lisa',
      last: 'Garcia',
      email: 'lisa.garcia@icloud.com',
      phone: '+1 (512) 555-0139',
      addr: '600 Congress Avenue, Austin, TX 78701',
    },
    {
      first: 'Robert',
      last: 'Nakamura',
      email: 'r.nakamura@gmail.com',
      phone: '+1 (808) 555-0121',
      addr: '2155 Kalakaua Avenue, Honolulu, HI 96815',
    },
    {
      first: 'Jennifer',
      last: "O'Brien",
      email: 'j.obrien@yahoo.com',
      phone: '+1 (404) 555-0166',
      addr: '191 Peachtree Street NE, Atlanta, GA 30303',
    },
  ];
  ...
  const caseRecords: Array<{
    id: string;
    caseRef: string;
    campaignId: string;
    status: string;
    consumer: (typeof consumerData)[0];
    submittedAt: Date;
    assignedTo?: string;
  }> = [];
  ...
  for (let i = 0; i < 18; i++) {
    ...
    const snapEnc = await encryptPII(
      JSON.stringify({
  ...
  const injuryCases = caseRecords
    .filter((c) => c.status !== 'rejected' && c.status !== 'withdrawn' && c.status !== 'duplicate')
    .slice(0, 6);
  const incidentRecords: Array<{ id: string; caseId: string; reviewId: string }> = [];

  const injuryScenarios = [
    {
      eventTypes: ['injury', 'product_failure'],
      answer: 'yes' as const,
      severity: 'moderate',
      narrative:
        'Child fell from stroller when front wheel detached while crossing a street intersection. Minor scrapes and bruising on both knees. Emergency room visit for evaluation; no fractures found. Stroller was being pushed at walking speed on a flat sidewalk.',
    },
    {
      eventTypes: ['product_failure'],
      answer: 'yes' as const,
      severity: 'minor',
      narrative:
        'Stroller wheel became loose while jogging in the park. Wheel wobbled and eventually came off, but I caught the stroller before it tipped. No injuries to child. Product had been used for approximately 3 months.',
    },
    {
      eventTypes: ['injury', 'fire'],
      answer: 'yes' as const,
      severity: 'serious',
      narrative:
        'Kettle continued heating after boiling. Plastic base melted and emitted smoke that triggered apartment fire alarm. Resident suffered minor smoke inhalation; treated at scene. Kitchen countertop had minor heat damage.',
    },
    {
      eventTypes: ['product_failure'],
      answer: 'yes' as const,
      severity: 'minor',
      narrative:
        'Kettle did not auto-shutoff. Water completely boiled away. Thermal fuse eventually activated but not before base became very hot to touch. No injuries; kettle disposed of immediately.',
    },
    {
      ...
      severity: 'serious',
      narrative:
        "Lollipop stick separated from candy portion while child was consuming it. Stick became lodged in child's throat for approximately 5 seconds before being dislodged by parent. Child taken to emergency room; no lasting injury observed. Product was purchased from local convenience store.",
    },
    {
      eventTypes: ['injury'],
      ...
      severity: 'moderate',
      narrative:
        'Consumer reported that after consuming product, child developed mild allergic reaction (hives around mouth). Unclear whether reaction was related to product or other food consumed at same time. Consumer unable to confirm ingredient list at time of incident.',
    },
  ];
  ...
  for (let i = 0; i < 6; i++) {
    ...
    await db
      ...
      .values({
        ...
        status: reviewStatus,
        reviewerId:
          reviewStatus !== 'pending' ? (staffIds['admin@koi-platform.com'] ?? null) : null,
        rationaleEncrypted:
          reviewStatus !== 'pending'
            ? await encryptNarrative(
                `Reviewed per 16 CFR § 1115. ${reviewStatus === 'filed' ? 'Filed with CPSC as potentially reportable within 24h of company notification.' : 'Determined non-reportable: incident did not meet statutory criteria for substantial product hazard.'}`,
              )
            : null,
  ...
  console.log(
    `    ✓ ${incidentRecords.length} incidents (${injuryScenarios.filter((s) => s.severity === 'serious').length} serious, ${injuryScenarios.filter((s) => s.severity === 'moderate').length} moderate, ${injuryScenarios.filter((s) => s.severity === 'minor').length} minor)`,
  );
  ...
  for (let i = 0; i < 10; i++) {
    ...
    await db
      ...
      .values({
        ...
        caseId: c.id,
        category:
          i % 3 === 0 ? 'product_photo' : i % 3 === 1 ? 'proof_of_purchase' : 'incident_evidence',
        categorySlot: 1,
  ...
  console.log('  ╠══════════════════════════════════════════╣');
  console.log(
    `  ║  Staff users:        ${String(Object.keys(staffIds).length).padStart(3)}                ║`,
  );
  console.log(
    `  ║  Campaigns:          ${String(allCampaigns.length).padStart(3)}                ║`,
  );
  console.log(`  ║  Claim drafts:       ${String(drafts.length).padStart(3)}                ║`);
  console.log(
    `  ║  Recall cases:       ${String(caseRecords.length).padStart(3)}                ║`,
  );
  console.log(
    `  ║  Incidents:          ${String(incidentRecords.length).padStart(3)}                ║`,
  );
