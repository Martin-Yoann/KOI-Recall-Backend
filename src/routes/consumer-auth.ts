...
interface ConsumerClaimSummary {
  ...
  /** What the review team asked the consumer to provide (need_info note). */
  infoRequest?: string;
...
function normalizeClaimStatus(
  ...
  // The review team asked for more information — surface it as a consumer
  // action item, never as silent progress ("verified" was misleading).
  if (caseStatus === 'need_info') return 'action_required';
  return 'submitted';
}
...
async function buildConsumerClaim(caseId: string): Promise<ConsumerClaimSummary | null> {
  ...
  // The most recent need_info transition note tells the consumer what to provide.
  const infoRequestEvent = [...eventRows]
    .reverse()
    .find(
      (eventRow) =>
        eventRow.eventType === 'case.status.transitioned' &&
        (eventRow.data as Record<string, unknown> | null)?.nextStatus === 'need_info' &&
  ...
  return {
    ...
    ...(infoRequest !== undefined ? { infoRequest } : {}),
