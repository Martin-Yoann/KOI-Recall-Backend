...
export function renderRefundCsv(
  rows: readonly {
  ...
  return (
    [
      CSV_HEADERS.join(','),
      ...rows.map((row) =>
        [row.caseReference, row.resolutionId, row.amountMinor, row.currency, row.resolutionVersion]
          .map(csvCell)
          .join(','),
      ),
    ].join('\r\n') + '\r\n'
...
export class RefundExportService {
  ...
  async export(input: RefundExportInput): Promise<RefundExportResult> {
    if (input.purpose.trim().length === 0 || input.purpose.length > 500)
      throw new Error('purpose must be 1-500 characters.');
    return this.handle.transaction(async (tx) => {
      const resolutions = await tx
        .select({
          id: caseResolutions.id,
          caseId: caseResolutions.caseId,
          amountMinor: caseResolutions.refundAmountMinor,
          currency: caseResolutions.currency,
          version: caseResolutions.version,
        })
        .from(caseResolutions)
        .where(
          and(
            eq(caseResolutions.approvedType, 'refund'),
            eq(caseResolutions.status, 'externally_completed'),
          ),
        );
      const ids = resolutions.map((row) => row.id);
      const prior =
        input.includeExported || ids.length === 0
          ? []
          : await tx
              .select({ caseResolutionId: refundExportItems.caseResolutionId })
              .from(refundExportItems)
              .where(inArray(refundExportItems.caseResolutionId, ids));
      ...
      const cases = await tx
        .select({ id: recallCases.id, reference: recallCases.publicReference })
        .from(recallCases)
        .where(inArray(recallCases.id, caseIds));
      const referenceById = new Map(cases.map((row) => [row.id, row.reference]));
      const rows = selected.map((row) => ({
        caseReference: referenceById.get(row.caseId) ?? row.caseId,
        resolutionId: row.id,
        amountMinor: row.amountMinor ?? 0,
        currency: row.currency ?? '',
        resolutionVersion: row.version,
      }));
      const csv = renderRefundCsv(rows);
      const sha256 = createHash('sha256').update(csv, 'utf8').digest('hex');
      const [batch] = await tx
        .insert(refundExportBatches)
        .values({
          requestedByStaffUserId: input.actorUserId,
          purpose: input.purpose,
          rowCount: rows.length,
          fileSha256: sha256,
        })
        .returning({ id: refundExportBatches.id });
      if (!batch) throw new Error('Refund export batch was not created.');
      await tx.insert(refundExportItems).values(
        selected.map((row, index) => ({
          exportBatchId: batch.id,
          caseResolutionId: row.id,
          resolutionVersion: row.version,
