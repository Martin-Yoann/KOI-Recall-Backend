...
export const refundExportBatches = pgTable(
  'refund_export_batches',
  {
    ...
    requestedByStaffUserId: uuid('requested_by_staff_user_id').notNull().references(() => staffUsers.id, { onDelete: 'restrict' }),
...
export const refundExportItems = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    exportBatchId: uuid('export_batch_id').notNull().references(() => refundExportBatches.id, { onDelete: 'restrict' }),
    caseResolutionId: uuid('case_resolution_id').notNull().references(() => caseResolutions.id, { onDelete: 'restrict' }),
    resolutionVersion: integer('resolution_version').notNull(),
    rowSha256: varchar('row_sha256', { length: 64 }).notNull(),
    createdAt: timestamp('created_at', { withTimezone: true, mode: 'date' }).notNull().defaultNow(),
  },
  (table) => [
    uniqueIndex('refund_export_items_batch_resolution_uidx').on(table.exportBatchId, table.caseResolutionId),
