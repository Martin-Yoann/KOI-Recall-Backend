...
| D03 | An inapplicable recall demands no declaration                                                       | Server: `tests/disposal-submit.integration.test.ts` — a submission with no approved instruction version succeeds and carries no `disposal` field. Browser: the consumer claim page was driven end to end against the local API (13 fields, both attestations, submit) and the submission went to one address only. The campaign had no approved content, so no task was created, `submitted.disposal` was absent, and the disposal branch at `claim-submit-wrapper.tsx:617` was skipped — the confirmation rendered by the same component showed no disposal section and asked for no declaration (`disposalSectionPresent` and `declarationAsked` both false). The surface driven is that component, not a separate demo: the confirmation text lives at lines 666-669 of the same file. |
